...
// ped.ro-style progressive reveal.
// Body text starts blurred; clicking a keyword capsule reveals its sentence
// after an intentional 350ms latency — the delay IS the concept.
...
const BLOCKS: Block[] = [
  ...
  },
  {
    ...
    lines: [
      [
        { type: "kw", id: "uiux", label: "UI/UX" },
        { type: "text", v: "에서는 정보가 사용자에게 도달하는 순간을 설계하고," },
      ],
      [
        { type: "kw", id: "graphic", label: "GRAPHIC" },
        { type: "text", v: " 디자인에서는 시각적 리듬과 여백을 통해 시선을 조율합니다." },
      ],
      [
        { type: "kw", id: "advertising", label: "ADVERTISING" },
        { type: "text", v: "에서는 메시지가 전달되는 타이밍에 따라 전혀 다른 결과가 만들어지며," },
      ],
      [
        { type: "kw", id: "xr", label: "XR" },
        { type: "text", v: "에서는 반응이 느껴지는 그 순간을 설계합니다." },
      ],
    ],
  },
  {
...
// keyword → block index it reveals
...
const REVEAL_DELAY = 350; // ms — intentional latency
...
export function AboutReveal() {
  ...
  // once all 9 keywords clicked → fade capsule borders
  useEffect(() => {
  ...
  const handleKeyword = useCallback((id: KId, e: React.MouseEvent) => {
    ...
    const delay = reducedMotion.current ? 0 : REVEAL_DELAY;
    setTimeout(() => {
      setRevealed(p => {
        const n = new Set(p);
        n.add(KW_BLOCK[id]);
      ...
      if (id === "experience" && !reducedMotion.current) {
        setPageFlash(true);
        setTimeout(() => setPageFlash(false), 700);
      }
    }, delay);
  }, [clicked, triggerEffect, addPosEffect]);
  ...
  return (
    <>
      ...
      {posEffects.map(e => (
        <div
          key={e.id}
          className={`pos-effect pos-effect--${e.kind}`}
      ...
      {/* Body */}
      <div
        className="px-6 sm:px-10 py-24 sm:py-36 mx-auto"
        style={{ maxWidth: 760 }}
      >
        <div className="flex flex-col" style={{ gap: "1.6em" }}>
          {BLOCKS.map(block => {
            const isRevealed = revealed.has(block.idx);
            return (
              <div key={block.idx} className="flex flex-col" style={{ gap: "0.1em" }}>
                {block.lines.map((line, li) => (
                  <p
                    key={li}
                    className="text-[18px] sm:text-[22px] leading-[1.75]"
                  ...
                  >
                    {line.map((part, pi) => {
                        ...
                        return (
                          <span
                            ...
                            style={{
                              ...
                              transition: rm ? "none" : "filter 600ms ease-out, color 600ms ease-out",
                            }}
                          >
                            {part.v}
                          </span>
                      ...
                      // keyword capsule
                      const kw = part;
                      const isClicked = clicked.has(kw.id);
                      const isAnimating = !!effects[kw.id];
                      const staggerIdx = STAGGER_ORDER.indexOf(kw.id);
                      return (
                        <Capsule
                          key={pi}
                          id={kw.id}
        ...
        {/* Progress counter */}
        <div
          className="mt-16 text-[11px] tabular-nums tracking-[0.06em]"
      ...
      <style>{`
        ...
        /* daun: horizontal stretch */
        @keyframes capStretch {
        ...
        /* LATENCY: letters appear one by one */
        @keyframes letterIn {
          from { opacity: 0.2; transform: translateY(-3px); }
          to   { opacity: 1;   transform: translateY(0); }
        }
        .eff-latency .lat-letter {
          animation: letterIn 150ms ease-out both;
          animation-delay: calc(var(--i) * 80ms);
        }

        /* timing: metronome dots */
        @keyframes metro {
        ...
        /* ADVERTISING: red sweep */
        @keyframes redSweepIn {
        ...
        .advert-bar {
          ...
          background: #ff4d3d; pointer-events: none;
          animation: redSweepIn 500ms ease-in-out;
        }

        /* XR: perspective tilt */
        @keyframes xrTilt {
          0%   { transform: perspective(200px) rotateY(0deg); }
          40%  { transform: perspective(200px) rotateY(15deg); }
        ...
        .pos-effect--dot {
          ...
          animation: dotFlash 250ms ease-out forwards;
        }
        @keyframes rippleExp {
        ...
        .pos-effect--ripple {
          ...
          animation: rippleExp 500ms ease-out forwards;
        }
      `}</style>
    </>
  );
...
function Capsule({
  id, label, isClicked, isAnimating, staggerIdx, allDone, onClick,
}: {
  id: KId;
  label: string;
  isClicked: boolean;
  ...
  return (
    <button
      ...
      disabled={isClicked}
      className={`kw-capsule ${isAnimating ? `eff-${id}` : ""}`}
      style={{
        display:        "inline-flex",
        alignItems:     "center",
        gap:            id === "timing" ? 6 : 0,
      ...
      onMouseEnter={(e) => {
        if (!isClicked) e.currentTarget.style.background = "rgba(255,255,255,0.07)";
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.background = "transparent";
      }}
    >
      {/* LATENCY letter-by-letter */}
      {id === "latency" ? (
        <span className={isAnimating ? "eff-latency" : ""} style={{ letterSpacing: "0.04em" }}>
          {"LATENCY".split("").map((ch, i) => (
            <span key={i} className="lat-letter" style={{ "--i": i } as React.CSSProperties}>
              {ch}
            </span>
          ))}
        </span>
          ...
          {isAnimating && (
            <span
              ...
              style={{
                ...
                padding: 4, gap: 1, pointerEvents: "none",
              }}
            >
        ...
        <>
          {label}
          {isAnimating && <span className="advert-bar" aria-hidden />}
        </>
      ) : (
        label
      )}
